...
def fetch_event_data_with_filters(category=None, start_date=None, end_date=None):
    # OLAP with Aggregate Functions:
    # This query aggregates data by grouping rows (GROUP BY) and calculating the average rating using AVG(). The GROUP BY
    # is applied on event-specific attributes and venue details.
    # It uses the COALESCE function to handle NULL values in ratings, ensuring a value of 0 is shown when no ratings exist.
    # The query returns aggregated event and venue details with the computed average rating for events.
    ...
    # Aggregate Query + Conditional Filtering
    # This query aggregates data similar to base_query_1 but applies conditional filtering based on the selected
    # category and/or date range.
    # It includes GROUP BY to calculate the average rating.
    # Dynamic filters are applied via Python (e.g., ec.category_name = %s for category and e.event_date BETWEEN %s AND
    # %s for date range).
    #
    base_query_2 = """
    SELECT 
    ...
    conditions = []
    params = []
    # Set Membership (for category) and Set Comparison (for date range)
    # Reasoning:
    #
    # Set Membership: The ec.category_name = %s condition checks if an event belongs to a specific category, making it
    # a set membership operation.
    # Set Comparison: The e.event_date BETWEEN %s AND %s condition compares the event date against a range, making it
    # a set comparison operation.
    ...
    # This query combines the results of base_query_1 (without filters) and base_query_2 (with filters) using the
    # INTERSECT operator.
    # INTERSECT ensures that only rows that are present in both queries are returned, effectively applying the
    # user-defined filters while preserving the base query structure.
